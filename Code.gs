...
function doPost(e) {
  try {
    ...
    return setCORSHeaders(
      ContentService.createTextOutput(JSON.stringify({ ok: true, result }))
...
function doGet(e) {
  // Maneja preflight y prueba de conexión
  if (e.parameter && e.parameter.action) {
    return doPost({ postData: { contents: JSON.stringify(e.parameter) } });
  }
  return setCORSHeaders(
    ContentService.createTextOutput(JSON.stringify({ ok: true, msg: 'Prodan API activa' }))
      .setMimeType(ContentService.MimeType.JSON)
  );
}
